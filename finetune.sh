...
for MODEL in $MODELS; do
  ...
  if [[ $(check_from_scratch "$MODEL") -eq 0 ]]; then
    ...
    if [[ "$NUM_LAYERS" = 'null' ]]; then
      ...
      TRAIN_COMMAND="python train_imagenet.py --network $MODEL"
    else
      # num-layers
      TRIMED_MODEL=$(trim_num_layers "$MODEL")
      TRAIN_COMMAND="python train_imagenet.py --network $TRIMED_MODEL --num-layers $NUM_LAYERS"
    fi
  else
    # fine-tuning
    # Determine LAYER_BEFORE_FULLC and IMAGE_SIZE
    LAYER_BEFORE_FULLC=$(get_layer_before_fullc "$MODEL")
    TRAIN_COMMAND="python fine-tune.py --pretrained-model $MODEL --layer-before-fullc $LAYER_BEFORE_FULLC --num-active-layers $NUM_ACTIVE_LAYERS"
  fi
  ...
  for OPTIMIZER in $OPTIMIZERS; do
    ...
    if [ "${PIPESTATUS[0]}" -eq 0 ]; then
      ...
      if [[ $TRAIN_ACCURACY_GRAPH_OUTPUT = 1 ]]; then
        ...
        python train_accuracy.py "$CONFIG_FILE" "$IMAGE" "$LOGS"
        if [[ $TRAIN_SLACK_UPLOAD = 1 ]]; then
          python slack_file_upload.py "$TRAIN_SLACK_CHANNELS" "$IMAGE"
        fi
      fi
      ...
      if [[ $AUTO_TEST = 1 ]]; then
        ...
        python predict.py "$CONFIG_FILE" "$MODEL_IMAGE_SIZE" "valid" "$MODEL_PREFIX" "$MODEL_EPOCH"
        ...
        if [[ $CONFUSION_MATRIX_OUTPUT = 1 ]]; then
          ...
          if [[ $TEST_SLACK_UPLOAD = 1 ]]; then
            python slack_file_upload.py "$TEST_SLACK_CHANNELS" "$IMAGE"
          fi
        fi
